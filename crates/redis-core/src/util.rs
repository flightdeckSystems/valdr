//! Utility functions ported from `src/util.c` + `src/util.h`.
//!
//! Provides:
...
//!
//! C source: `src/util.c` (1671 lines, 54 functions) + `src/util.h`.
...
// `get_hash_seed_from_string` and `get_random_bytes`.  sha256.h is marked SKIP
...
/// C: `typedef enum { LD_STR_AUTO, LD_STR_HUMAN, LD_STR_HEX } ld2string_mode`
...
///
/// C: file-scope `static unsigned char seed[64]`, `static int seed_initialized`,
...
///
/// C: `static int stringmatchlen_impl(...)`, util.c:63-189.
...
///
/// Returns `true` if `string` matches `pattern`.  `nocase = true` makes the
/// comparison case-insensitive.
///
/// C: `int stringmatchlen(const char *pattern, int patternLen, const char *string,
///     int stringLen, int nocase)`, util.c:191-194.
...
///
/// C: `int stringmatch(const char *p, const char *s, int nocase)`, util.c:196-198.
...
/// that don't end with `*`.
///
/// C: `int prefixmatchlen(...)`, util.c:200-212.
...
///
/// C: `int stringmatchlen_fuzz_test(void)`, util.c:215-228.
...
///
/// C: `unsigned long long memtoull(const char *p, int *err)`, util.c:238-291.
...
///
/// C: `const char *mempbrk(const char *s, size_t len, const char *chars, size_t charslen)`,
/// util.c:296-303.
...
/// In-place byte translation: replace each byte in `from` with the
/// corresponding byte in `to` across the buffer `s`.
///
/// C: `char *memmapchars(char *s, size_t len, const char *from, const char *to,
///     size_t setlen)`, util.c:308-318.
...
///
/// C: `uint32_t digits10(uint64_t v)`, util.c:322-340.
...
///
/// C: `uint32_t sdigits10(int64_t v)`, util.c:343-351.
...
///
/// C: `int ull2string(char *dst, size_t dstlen, unsigned long long value)`,
/// util.c:396-430.  The Rust version writes into a `&mut [u8]` slice and does
...
///
/// C: `int ll2string(char *dst, size_t dstlen, long long svalue)`, util.c:356-386.
...
///
/// C: `static int string2llScalar(const char *s, size_t slen, long long *value)`,
/// util.c:571-635.  The AVX-512 SIMD path (`string2llAVX512`) is intentionally
...
///
/// C: `int string2ull(const char *s, size_t slen, unsigned long long *value)`,
/// util.c:670-682.
...
///
/// C: `int string2l(const char *s, size_t slen, long *lval)`, util.c:687-696.
...
///
/// C: `static int safe_is_c_in_range(char c, char start, char end)`, util.c:699-702.
#[inline(always)]
...
///
/// C: `static int base_16_char_type(char c)`, util.c:704-709.
...
///
/// Parses hex digits (uppercase or lowercase) until a non-hex byte or the
/// end of the slice.  Returns `Err(())` on overflow, `Ok(value)` on success.
///
/// C: `int string2ul_base16_async_signal_safe(const char *src, size_t slen,
///     unsigned long *result_output)`, util.c:715-732.
/// PORT NOTE: C returns 1 on success and -1 on overflow; translated to
...
///
/// C: `int string2ld(const char *s, size_t slen, long double *dp)`, util.c:741-759.
...
///
/// C: `int string2d(const char *s, size_t slen, double *dp)`, util.c:768-778.
...
///
/// C: `int double2ll(double d, long long *out)`, util.c:782-807.
...
///
/// C: `int d2string(char *buf, size_t len, double value)`, util.c:814-845.
...
///
/// C: `int fixedpoint_d2string(char *dst, size_t dstlen, double dvalue,
///     int fractional_digits)`, util.c:867-965.
...
///
/// C: `int trimDoubleString(char *buf, size_t len)`, util.c:968-979.
...
///
/// C: `int ld2string(char *buf, size_t len, long double value,
///     ld2string_mode mode)`, util.c:990-1053.
...
///
/// C: `void getHashSeedFromString(unsigned char *seed_array, size_t outlen,
///     const char *value)`, util.c:1057-1067.
...
///
/// C: `int version2num(const char *version)`, util.c:1072-1091.
...
/// time-/pid-based seed.
///
/// C: `static void initializeRandomSeed(void)`, util.c:1097-1117.
...
///
/// C: `void setRandomSeedCString(char *seed_str, size_t len)`, util.c:1121-1127.
...
///
/// C: `void getRandomBytes(unsigned char *p, size_t len)`, util.c:1149-1190.
...
pub fn get_random_bytes(p: &mut [u8]) {
    ...
    // C: util.c:1156-1189  — two rounds of sha256 (IKEY xor 0x36, OKEY xor 0x5C)
...
///
/// C: `void getRandomHexChars(char *p, size_t len)`, util.c:1196-1202.
...
///
/// C: `sds getAbsolutePath(char *filename)`, util.c:1211-1251.
...
/// For now, returns 0.  Phase B should call `libc::timezone` or use the
/// `chrono` / `time` crate.
///
/// C: `long getTimeZone(void)`, util.c:1257-1267.
...
///
/// C: `int pathIsBaseName(char *path)`, util.c:1273-1275.
...
///
/// C: `int fileExist(char *filename)`, util.c:1277-1280.
...
///
/// C: `int dirExists(char *dname)`, util.c:1282-1285.
...
///
/// C: `int dirCreateIfMissing(char *dname)`, util.c:1287-1297.
...
///
/// C: `int dirRemove(char *dname)`, util.c:1299-1348.
...
///
/// C: `sds makePath(char *path, char *filename)`, util.c:1350-1352.
...
///
/// C: `int fsyncFileDir(const char *filename)`, util.c:1362-1400.
...
///
/// C: `int reclaimFilePageCache(int fd, size_t offset, size_t length)`,
/// util.c:1403-1417.
...
///
/// C: `char *fgets_async_signal_safe(char *dest, int buff_size, int fd)`,
/// util.c:1427-1441.
...
///
/// C: `static char *u2string_async_signal_safe(int _base, uint64_t val, char *buf)`,
/// util.c:1445-1452.  The C version fills right-to-left from the end of a
...
///
/// C: `static char *i2string_async_signal_safe(int base, int64_t val, char *buf)`,
/// util.c:1454-1501.  Two's-complement hex for negative base-16 values;
...
///
/// C: `int vsnprintf_async_signal_safe(char *to, size_t size,
///     const char *format, va_list ap)`, util.c:1517-1590.
...
///
/// C: `int snprintf_async_signal_safe(char *to, size_t n, const char *fmt, ...)`,
/// util.c:1592-1599.
...
///
/// C: `long long ustime(void)`, util.c:1602-1622.
///
/// PERF(port): C has a fast path using a hardware monotonic clock via
/// `getMonotonicUs` (from `monotonic.c`, phase `defer`).  This version always
...
///
/// C: `mstime_t mstime(void)`, util.c:1625-1627.
...
///
/// C: `void writePointerWithPadding(unsigned char *buf, const void *ptr)`,
/// util.c:1631-1636.
...
/// `"`) to `s`.
///
/// C: `sds escapeJsonString(sds s, const char *p, size_t len)`, util.c:1642-1658.
...
///
/// C: `uint64_t wangHash64(uint64_t hash)`, util.c:1661-1670.
...
// ──────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:        src/util.c  (1671 lines, 54 functions) + src/util.h
