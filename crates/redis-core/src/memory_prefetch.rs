...
//! # Design overview
//!
//! 1. `prefetch_commands_batch_init` allocates the singleton batch from
//!    `server.prefetch_batch_max_size`.
//! 2. `add_command_to_batch_and_process_if_full` is called once per incoming
//!    client on the I/O-thread path; it adds the client and its keys to the
...
/// Placeholder for `hashtableIncrementalFindState` from `hashtable.h`.
///
...
/// Placeholder for `hashtable *` from `hashtable.h`.
///
...
///
/// C: `PrefetchState` — memory_prefetch.c:15-19
...
///
/// C: `KeyPrefetchInfo` — memory_prefetch.c:21-24
...
///
/// C: `PrefetchCommandsBatch` — memory_prefetch.c:27-39
///
...
struct PrefetchCommandsBatch {
    ...
    ///
    /// C: `void **keys` — raw argv object pointers, later recast to sds data.
    ...
    ///
    /// C: `KeyPrefetchInfo *prefetch_info`
...
impl PrefetchCommandsBatch {
    ...
    ///
    /// C: body of `prefetchCommandsBatchInit` — memory_prefetch.c:65-72
    ...
    ///
    /// C: `moveToNextKey` — memory_prefetch.c:87-89
    ...
    ///
    /// C: `getNextPrefetchInfo` — memory_prefetch.c:98-106
    ...
    ///
    /// C: `resetCommandsBatch` — memory_prefetch.c:170-176
    fn reset(&mut self) {
        self.cur_idx = 0;
        self.keys_done = 0;
        self.key_count = 0;
...
///
/// C: `markKeyAsdone` — memory_prefetch.c:91-95
///
...
///
/// C: `initBatchInfo` — memory_prefetch.c:108-120
///
    ...
    for i in 0..batch.key_count {
        let table_present_and_non_empty = match &batch.keys_tables[i] {
            ...
            Some(_t) => {
                ...
                // C: if (!tables[i] || hashtableSize(tables[i]) == 0)
...
///
/// C: `prefetchEntry` — memory_prefetch.c:122-133
///
...
fn prefetch_entry(batch: &mut PrefetchCommandsBatch, idx: usize) {
    ...
    // C: if (hashtableIncrementalFindStep(&info->hashtab_state)) { … }
    ...
    // C: } else if (server.io_threads_num >= server.min_io_threads_copy_avoid) {
...
///
/// C: `prefetchValue` — memory_prefetch.c:136-146
///
...
fn prefetch_value(batch: &mut PrefetchCommandsBatch, idx: usize) {
    ...
    // C: if (hashtableIncrementalFindGetResult(&info->hashtab_state, &entry)) { … }
...
///
/// C: `hashtablePrefetch` — memory_prefetch.c:158-168
...
///
/// C: `prefetchCommands` — memory_prefetch.c:181-214
///
...
fn prefetch_commands(batch: &mut PrefetchCommandsBatch) {
    // C: memory_prefetch.c:183-201
    // Step 1: prefetch argv object headers for each client (skipping argv[0] — the
...
///
/// C: `addCommandToBatch` (static) — memory_prefetch.c:246-257
///
/// TODO(port): `getKeysFromCommand` / `GetKeysResult` are not yet ported
/// (they live in `commands.c` / `server.c`).  The function is a skeleton.
///
...
///
/// C: `prefetchCommandsBatchInit` — memory_prefetch.c:57-72
///
...
///
/// C: `freePrefetchCommandsBatch` — memory_prefetch.c:43-55
...
///
/// Returns `true` (C_OK) unconditionally.  If a batch with clients is in
/// progress, the resize is deferred until the batch drains.
///
/// C: `onMaxBatchSizeChange` — memory_prefetch.c:74-84
///
...
///
/// 1. On the first (non-recursive) invocation: runs `prefetch_commands` to
...
///
/// C: `processClientsCommandsBatch` — memory_prefetch.c:217-243
///
/// TODO(port): `process_pending_command_and_input_buffer(client)` and
/// `before_next_client(client)` — these live in `networking.c`/`server.c`
...
pub fn process_clients_commands_batch() -> Result<(), RedisError> {
    ...
    // C: if (batch->executed_commands == 0) prefetchCommands();
    ...
    // C: memory_prefetch.c:227-235
    // for i in 0..batch.client_count {
    //     let Some(client_id) = batch.clients[i] else { continue };
    //     batch.clients[i] = None;   // null out before recursive call
    ...
    // C: memory_prefetch.c:240-242 — handle deferred resize.
...
///
/// C: `addCommandToBatchAndProcessIfFull` — memory_prefetch.c:263-290
///
...
pub fn add_command_to_batch_and_process_if_full(
    ...
    BATCH.with(|cell| {
        ...
        // C: memory_prefetch.c:275-280 — queued pipeline commands.
...
///
/// C: `removeClientFromPendingCommandsBatch` — memory_prefetch.c:293-302
...
// ──────────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:        src/memory_prefetch.c  (303 lines, 10 functions)
