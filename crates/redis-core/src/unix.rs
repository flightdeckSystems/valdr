...
//!
//! Port of `unix.c` (222 lines, 18 functions). Implements `ConnectionTypeTrait`
...
//! # C source reference
//!
//! C: `src/unix.c` (222 lines)
//! C: `src/connection.h` (merged; type definitions only)
...
///
/// C: `IOV_MAX` from `<limits.h>` (1024 on Linux and macOS).
...
///
/// C: `static ConnectionType CT_Unix` (unix.c:170-217)
...
impl ConnectionTypeTrait for UnixConnectionType {
    ...
    ///
    /// C: `connUnixGetType` → `CONN_TYPE_UNIX`
    ...
    ///
    /// C: `connUnixIsLocal` (unix.c:49-53) — always returns `1`.
    ...
    ///
    /// C: `connUnixListen` (unix.c:55-78)
    ///
    /// The C implementation calls `unlink(addr)` to remove any stale socket
    /// file, then `anetUnixServer(neterr, addr, perm, backlog, group)` to
    ...
    ///
    /// C: `connUnixCloseListener` → `connectionTypeTcp()->closeListener(listener)`
    ///
    ...
    fn close_listener(&self, listener: &mut ConnListener) {
        ...
        // C: connectionTypeTcp()->closeListener(listener)
    ...
    ///
    /// C: `connCreateUnix` (unix.c:84-91)
    fn conn_create(&self) -> Connection {
        // C: conn->type = &CT_Unix; conn->fd = -1; conn->iovcnt = IOV_MAX;
    ...
    ///
    /// C: `connCreateAcceptedUnix` (unix.c:93-99)
    fn conn_create_accepted(&self, fd: i32) -> Connection {
        // C: conn = connCreateUnix(); conn->fd = fd; conn->state = CONN_STATE_ACCEPTING;
    ...
    ///
    /// C: `connUnixShutdown` → `connectionTypeTcp()->shutdown(conn)`
    ///
    ...
    ///
    /// C: `connUnixClose` → `connectionTypeTcp()->close(conn)`
    ///
    ...
    ///
    /// C: `CT_Unix.blocking_connect = NULL`
    fn blocking_connect(
        &self,
        _conn: &mut Connection,
        _addr: &[u8],
    ...
    ///
    /// C: `connUnixAccept` → `connectionTypeTcp()->accept(conn, accept_handler)`
    ///
    ...
    ///
    /// C: `connUnixWrite` → `connectionTypeTcp()->write(conn, data, data_len)`
    ///
    ...
    ///
    /// C: `connUnixWritev` → `connectionTypeTcp()->writev(conn, iov, iovcnt)`
    ///
    ...
    ///
    /// C: `connUnixRead` → `connectionTypeTcp()->read(conn, buf, buf_len)`
    ///
    ...
    ///
    /// C: `connUnixSetWriteHandler` → `connectionTypeTcp()->set_write_handler(conn, func, barrier)`
    ///
    ...
    ///
    /// C: `connUnixSetReadHandler` → `connectionTypeTcp()->set_read_handler(conn, func)`
    ///
    ...
    ///
    /// C: `connUnixGetLastError` (unix.c:154-156) — returns `strerror(conn->last_errno)`.
    ///
    ...
    /// The result is an OS error description string (not Redis data), so the
    ...
    ///
    /// C: `connUnixSyncWrite` (unix.c:158-160) → `syncWrite(conn->fd, ptr, size, timeout)`
    ///
    ...
    ///
    /// C: `connUnixSyncRead` (unix.c:162-164) → `syncRead(conn->fd, ptr, size, timeout)`
    ///
    ...
    ///
    /// C: `connUnixSyncReadLine` (unix.c:166-168) → `syncReadLine(conn->fd, ptr, size, timeout)`
    ///
...
///
/// C: `connUnixAcceptHandler` (unix.c:101-120)
///
...
pub fn unix_accept_handler(
    ...
    while remaining > 0 {
        ...
        //
        // C: cfd = anetUnixAccept(server.neterr, fd);
        //    if (cfd == ANET_ERR) {
...
///
/// C: `RedisRegisterConnectionTypeUnix` (unix.c:219-221)
/// C: `return connTypeRegister(&CT_Unix);`
///
...
/// per PORTING.md §7 naming conventions).
...
// ──────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:        src/unix.c  (222 lines, 18 functions)
