//! FIFO queue for Valkey — Phase A port of `fifo.c` / `fifo.h`.
//!
...
//! giving a zero-overhead union of `(next: *mut FifoBlock, index: u3)` in the
//! same 8-byte word.
//!
...
//! - A safe alternative (store index separately) costs 8 bytes per block and
...
//! 7× fewer allocations vs `adlist`) can be restored in Phase B once the
//! architect decides on the `unsafe` budget for `redis-core`.
//!
//! # C source reference
//! `src/fifo.c` (354 lines, 7 functions) + `src/fifo.h` (74 lines)
//! Phase assignment: **defer** (harness/file-deps.tsv line 76–77).
...
///
/// This is a generic Rust port of the C `fifo` type from `fifo.h`.  The C type
...
impl<T> Fifo<T> {
    ...
    ///
    /// PORT NOTE: `std::mem::replace` achieves the same O(1) semantics for the
...
impl<T> Drop for Fifo<T> {
    ...
    /// remaining item.  When `T` is a raw pointer this is a no-op, matching the
    /// C `fifoRelease` which frees blocks but not items.
    ///
    /// # C: `void fifoRelease(fifo *q)`
    /// ```c
    /// // walks the block chain and zfree()s each block, then zfree(q)
    /// // does NOT free items
    /// ```
    fn drop(&mut self) {
...
// ──────────────────────────────────────────────────────────────────────────────
//
// The following mirrors the C internal layout.  It is NOT compiled into the
// Phase A binary but is retained so Phase B implementors can restore the
...
// ──────────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:        src/fifo.c  (354 lines, 7 functions) + src/fifo.h (74 lines)
