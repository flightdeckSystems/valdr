...
//!
//! Port of `src/timeout.c` (Valkey, 194 lines, 8 functions).
//!
...
//! ## Phase notes
//!
//! The radix-tree timeout table (`server.clients_timeout_table` in C) maps to
...
/// shared `redis-core::util` module once those commands are ported and the
...
/// time in milliseconds; it is passed in to avoid repeated syscalls across the
/// cron loop's client sweep.
///
/// Returns `true` if the client was terminated and **must not be accessed
/// again** by the caller.
///
/// PORT NOTE: The C implementation calls `freeClient(c)` which removes the
...
///
/// C: timeout.c:53-74, clientsCronHandleTimeout
pub fn clients_cron_handle_timeout(
    server: &mut RedisServer,
    client: &mut Client,
    now_ms: i64,
) -> bool {
    // C: time_t now = now_ms / 1000;
    ...
    {
        // C: serverLog(LL_VERBOSE, "Closing idle client"); freeClient(c);
...
///
/// C: timeout.c:92-96, encodeTimeoutKey
...
///
/// C: timeout.c:100-104, decodeTimeoutKey
...
///
/// C: timeout.c:109-115, addClientToTimeoutTable
pub fn add_client_to_timeout_table(server: &mut RedisServer, client: &mut Client) {
    ...
    // (redis-ds, Phase 4/5).
    // C: if (raxTryInsert(server.clients_timeout_table, buf, sizeof(buf), NULL, NULL))
...
///
/// C: timeout.c:119-126, removeClientFromTimeoutTable
...
///
/// C: timeout.c:130-148, handleBlockedClientsTimeout
pub fn handle_blocked_clients_timeout(server: &mut RedisServer) {
    ...
    //       }
    //       table.remove(&key);
    //       // Re-seek to start (C: raxSeek(&ri, "^", NULL, 0)) because
...
///
/// C: timeout.c:158-193, getTimeoutFromObjectOrReply
pub fn get_timeout_from_object_or_reply(
    ...
    let tval: i64 = match unit {
        TimeUnit::Seconds => {
            // C: getLongDoubleFromObjectOrReply(c, object, &ftval,
            ...
            // C: if (ftval > (long double)LLONG_MAX) { addReplyError(...); return C_ERR; }
            if ftval_ms > i64::MAX as f64 {
                return Err(RedisError::runtime(b"timeout is out of range"));
            }

            // C: tval = (long long)ceill(ftval)
            ftval_ms.ceil() as i64
        }

        TimeUnit::Milliseconds => {
            // C: getLongLongFromObjectOrReply(c, object, &tval,
    ...
    // C: if (tval < 0) { addReplyError(c, "timeout is negative"); return C_ERR; }
    if tval < 0 {
        return Err(RedisError::runtime(b"timeout is negative"));
    }

    // C: if (tval > 0) { if (tval > LLONG_MAX - now) → overflow; tval += now; }
    if tval > 0 {
...
/// on `RedisObject`, ported from `getLongDoubleFromObject` in `object.c`.
///
...
///
/// Stub — the real implementation belongs in `redis-core::object` as a method
/// on `RedisObject`, ported from `getLongLongFromObject` in `object.c`.
///
...
// ──────────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:        src/timeout.c  (194 lines, 8 functions)
