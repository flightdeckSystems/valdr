...
def resolve_deny_tags(profile):
    import importlib.util
    spec = importlib.util.spec_from_file_location("tcl_survey", MAIN / "harness/oracle/tcl-survey.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return list(mod.DENY_TAG_PROFILES[profile])


_SUITE = json.loads(SUITE_CONFIG.read_text())
DISCOVER_GLOBS = _SUITE["discover"]
EXCLUDE_FILES = set(_SUITE["exclude_files"])
LABEL_OVERRIDES = _SUITE["labels"]
DENY_TAGS = resolve_deny_tags(_SUITE["profile"])
...
# Every file invocation gets a UNIQUE stepped port. Reusing one baseport across
# rapid sequential files fills the harness client-port window with TIME_WAIT
# sockets -> no-summary. (slot picks the isolated test dir; port is independent.)
_port_counter = [0]
...
def next_port():
    with _port_lock:
        n = _port_counter[0]
        _port_counter[0] += 1
    span = max(1, (44000 - BASE_PORT) // 100)
    return BASE_PORT + (n % span) * 100
...
def render_view(selected, cache):
    ...
    payload = {
        ...
        "method": (
            ...
            "with per-file write limits. single-node-external profile (deny needs:repl, repl, "
            "needs:debug, cluster, needs:cluster — external:skip tests kept), no --durable."
        ),
...
def main():
    ...
    ap.add_argument("--newest-first", action="store_true", help="Measure most-recent commits first (the high-value points appear sooner).")
